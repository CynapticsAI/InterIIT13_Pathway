...
from Helper.Flare_Setup import *
...
def string_streamer(text, chunk_size=1, delay=0.01):
...
# st.header("💬 Dynamic Active Retrieval(Open source) ChatBot", divider=True)
st.header("💬 FLARE ChatBot", divider=True)
...
if user_input := st.chat_input("Type your message here..."):
    st.session_state["current_response_list"] = current_response_list
    st.session_state["subquery_list"] = subquery_list
    ...
    with st.spinner("Generating response..."):
        # Generate a response using the Agent function
        response, tool_calls_dict = Agent(user_input)
        st.session_state["tools_dict"] = tool_calls_dict
