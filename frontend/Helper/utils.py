...
class BasicGenerator:
    def __init__(self, model_name_or_path):
        ...
        # self.model = AutoModelForCausalLM.from_pretrained(model_name_or_path, 
                    # trust_remote_code = "falcon" in model_name_or_path, torch_dtype="auto").to("cuda")
        self.model = AutoModelForCausalLM.from_pretrained(model_name_or_path, 
                    trust_remote_code = "falcon" in model_name_or_path, torch_dtype=torch.float16).to("cuda")
    ...
    def correct_prompt(self, prompt):
        # messages = [
        #     {"role": "system", "content": "You are an intelligent assistant who follows the users instruction correctly without any deviation!"},
    ...
    def generate(self, input_text, max_length, return_logprobs=False):
        input_text = self.correct_prompt(input_text)
    ...
    def generate_attn(self, input_text, max_length, solver="max", use_entropy = False, use_logprob = False):
        ...
        # print("_"*100)
        # print(input_ids.shape, max_length)
        # print("_"*100)
        outputs = self.model.generate(
            # input_ids = input_ids, 
            # attention_mask = attention_mask,
...
class AttnWeightRAG(BasicRAG):
    ...
    def inference(self, question, demo, case):
        ...
        while True:
            context = []
            context.append(text) # current text
            # print(f"`current text is `\n{text}")
            old_len = len(text)
            # prompt = "".join([d["case"]+"\n" for d in demo])
            prompt = "You are an Intelligent Agent and You answer the given question, you can refer to the given context to answer the question. Only answer the question output nothing else, just a bit descriptive like it should seem like question is being answered.\n"
            if text == "":
                docs = self.retrieve(question, topk=self.retrieve_topk)
                context.append("\n".join(docs)) # first retrieved docs
                prompt += "Context:\n"
                for i, doc in enumerate(docs):
                    prompt += f"[{i+1}] {doc}\n"
                # prompt += "Answer in the same format as before.\n"
            ...
            # print(f"`new_text is `\n{new_text}")
            context.append(new_text) # text generated by llm
            ...
            # print(f"`ptext is `\n{ptext}")
            # print(f"`Is hallucinating: `{hallucination}")
            context.append(hallucination) # is llm hallucinating
            
            if not hallucination:
                text = text.strip() + " " + new_text.strip()
            else:
                context.append(ptext) # partial text as model is hallucinating
                ...
                # print(f"`retrieve_question is `\n{retrieve_question}")
                context.append(retrieve_question) # question used for further retrieval
                docs = self.retrieve(retrieve_question, topk=self.retrieve_topk)
                # print(f"`retriever docs are:`")
                # print(docs)
                context.append("\n".join(docs)) # retrieved docs

                # prompt = "".join([d["case"]+"\n" for d in demo])
                prompt = "You are an Intelligent Agent and You answer user questions, you can refer to the given context to answet the question."
                prompt += "Context:\n"
                for i, doc in enumerate(docs):
                    prompt += f"[{i+1}] {doc}\n"
                # prompt += "Answer in the same format as before.\n"
                ...
                # prompt += case + " " + text + " " + ptext.strip()
                new_text, _, _ = self.generator.generate(prompt, self.generate_max_length)
                # print(f"`new_text after retrieving is `\n{new_text}")
                context.append(new_text) # text generated by llm
                ...
                # print(f"`new_text after getting top sentence `\n{new_text}")
                context.append(new_text) # generated text after modifying
            ...
            # 判断 token 的个数要少于 generate_max_length 
            tokens_count = len(self.generator.tokenizer.encode(text))
            if tokens_count > self.generate_max_length or len(text) <= old_len or "the answer is" in text or self.is_correct_answer(question, text):
                break

        return text, contexts

    def is_correct_answer(self, question, text):
        # answer = get_answer(question, text)
        prompt = f"Is the given answer fulfills the requirement for question completely? Only give the ONE WORD answer 'YES' if it answer fulfills the question or output 'NO'. Dont check the authenticity of the answer just check if answer answers the given question even if the answer is right or wrong.\nQUESTION:{question}\nANSWER:{text}"
        ...
        generated_tokens = outputs[:, input_length:]
        answer = self.generator.tokenizer.decode(generated_tokens[0], skip_special_tokens=True)
    ...
    def change_model(self, model_name_or_path):
        self.generator.model = AutoModelForCausalLM.from_pretrained(model_name_or_path, 
                    trust_remote_code = "falcon" in model_name_or_path, torch_dtype=torch.float16).to("cuda")
